    ...
    void search(int key)
    {
        if (!isEmpty())
        {
            ...
            for (int i = 0; i < size - 1; i++)
            {
                ...
                {
                    cout << "\n"
                         << key << " is present in the Queue" << endl;
                    found = true;
                }
            }
            if (found == false)
            {
                cout << "\n"
                     << key << " Not present in the Queue" << endl;
            }
        }
        else
        {
...
int main()
{
    ...
    q1.enQueue(60);
    q1.frontElement(); // 20
    q1.displayQueue(); // 30 20 50 40 60
    q1.deQueue();      // 20
    q1.displayQueue(); // 20 50 40 60
    q1.search(60);     // present
    q1.search(100);    // Not present
    return 0;
}